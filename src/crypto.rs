...
#[derive(Clone)]
pub struct CryptoContextEx {
    pub public_key: String,
    key_pair: Arc<Mutex<ec::EcKey>>,
}

impl CryptoContextEx {
    pub fn new() -> Option<Self> {
        ...
        let public_key_bytes;
        {
            ...
            public_key_bytes = public_key.to_bytes(&group, ec::POINT_CONVERSION_COMPRESSED, &mut ctx).unwrap();
        }

        Some(CryptoContextEx {
    ...
    pub fn ecdh_derive_keys(&self, peer_key_bytes: &[u8]) -> Vec<u8> {
        let mut bn = BigNumContext::new().unwrap();
        let group = ec::EcGroup::from_curve_name(nid::X9_62_PRIME256V1).unwrap();
        let peer_ecpoint = ec::EcPoint::from_bytes(&group, peer_key_bytes, &mut bn).unwrap();
        let peer_eckey = ec::EcKey::from_public_key(&group, &peer_ecpoint).unwrap();
        let peer_pkey = pkey::PKey::from_ec_key(peer_eckey).unwrap();

        let key_pair = self.key_pair.lock().unwrap();
        let local_eckey = (*key_pair).from_self().unwrap();
        let local_pkey = pkey::PKey::from_ec_key(local_eckey).unwrap();

        let mut ctx = pkey::PKeyCtx::from_pkey(&local_pkey).unwrap();
        ctx.derive(&peer_pkey).unwrap()
    }
    ...
    pub fn encrypt(&self,
        ...
        let shared_key = self.ecdh_derive_keys(&peer_key_bytes);
...
mod tests {
    ...
    #[test]
    fn ecdh_encrypt_and_decrypt_payload_ex() {
        use super::CryptoContextEx;

        let local = CryptoContextEx::new().unwrap();
        let peer = CryptoContextEx::new().unwrap();
        let input = String::from("testing ecdh");
        let auth = None;
        let rs = 4096;
        let encrypt_data = local.encrypt(&peer.public_key, input.clone(), &auth, rs).unwrap();
    ...
    #[test]
    fn ecdh_encrypt_and_decrypt_payload_using_auth_ex() {
        use super::CryptoContextEx;
        use rustc_serialize::base64::{ ToBase64, STANDARD };
